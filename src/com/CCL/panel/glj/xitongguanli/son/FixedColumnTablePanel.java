...
public class FixedColumnTablePanel extends MPanel {// 点击表格，是点击的行被选中
...
//				}
		...
		// fixedColumnTable.setOpaque(false);

		// 设置固定表格的列宽

		for (int i = 0; i < fixwidth.length; i++) {
			fixedColumnTable.getColumnModel().getColumn(i)
					.setPreferredWidth(fixwidth[i]);
		}
...
//				}
		...
		for (int i = 0; i < floatwidth.length; i++) {
			floatingColumnTable.getColumnModel().getColumn(i)
					.setPreferredWidth(floatwidth[i]);
		}
	...
	class MTable extends JTable {
		...
		public void setActionCommand(String string) {
			...
			((MTable) getModel()).setActionCommand(string);
	...
	public MTable getFixedColumnTable() {
		return fixedColumnTable;
	}

	public void setFixedColumnTable(MTable fixedColumnTable) {
		this.fixedColumnTable = fixedColumnTable;
	}

	public MTable getFloatingColumnTable() {
		return floatingColumnTable;
	}

	public void setFloatingColumnTable(MTable floatingColumnTable) {
