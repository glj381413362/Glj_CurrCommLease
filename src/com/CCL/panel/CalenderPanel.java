			...
			if (e.getClickCount() == 2) {// 处理鼠标双击
				...
				if(!mainFrame.getBackPanel().getName().equals("自行车"))
				{
					paneA2b.removeAll();
			     	 ...
			     	 riYinYeSelect.getTxt_stime().setText(sdf.format(calendar.getTime()));
			     	 riYinYeSelect.getTxt_etime().setText(sdf.format(calendar.getTime()));
				...
				if(mainFrame.getFristLabel().getText().equals("销售趋势分析"))
				{
					
					 Calendar x = Calendar.getInstance();
						x.setTime(calendar.getTime());// stime
						x.add(Calendar.DAY_OF_YEAR, 0);
						String stim = DateUtil.formatDate1(x.getTime());
						x.add(Calendar.DAY_OF_YEAR, 0);
						String etim = DateUtil.formatDate1(x.getTime());
						if(i==0)
				     	 {
							XiaoShouQuShi.getTxt_stime().setText(stim);
					     	 first = stim;
					     	XiaoShouQuShi.getTxt_stime().repaint();
					     	i=1;
				     	 }
						else{
				     		XiaoShouQuShi.getTxt_stime().setText(first);
				     		XiaoShouQuShi.getTxt_etime().setText(etim);
				     		XiaoShouQuShi.getTxt_stime().repaint();
				     		XiaoShouQuShi.getTxt_etime().repaint();
				     		i=0;
				     	 }
				...
				/*	if(!mainFrame.getBackPanel().getName().equals("自行车"))
					{
						paneA2b.removeAll();
				     	  sellpanel.removeAll();
						 XiaoShouQuShi  xiaoShouQuShi = new XiaoShouQuShi("",sellpanel);;
						 Calendar x = Calendar.getInstance();
							x.setTime(calendar.getTime());// stime
							x.add(Calendar.DAY_OF_YEAR, 0);
							String stim = DateUtil.formatDate1(x.getTime());
				     	 ...
				     	 if(i!=0){
				     		xiaoShouQuShi.getTxt_stime().setText(first);
				     		xiaoShouQuShi.getTxt_etime().setText(etim);
				     		System.out.println("fdhsssssssssssssssssssssssssssssss");
				     		i=0;
				     	 }
				     	i=1;
				     	
				     	
				     	 sellpanel.add(xiaoShouQuShi);
				    	  mainFrame.getBackPanel().add(mainFrame.getLabel_1());
				    	  mainFrame.getFristLabel().setBounds(133, 38, 123, 15);
				    	  mainFrame.getBackPanel().add( mainFrame.getFristLabel());
				          MJLable mainR = new MJLable("images/beijin_r_lbl_end.gif", 300, 300);
						 ...
						 Calendar x = Calendar.getInstance();
							...
							String stim = DateUtil.formatDate1(x.getTime());
							System.out.println(x.getTime().toString()+stim);
				     	 ...
				     	 sellpanel.add(xiaoShouQuShi);
				     	mainFrame.getFristLabel().setText("销售趋势分析");
					}
					mainFrame.repaint();*/
				}
